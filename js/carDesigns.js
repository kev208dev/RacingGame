...
// 부드러운 원형 텍스처 (smoke_puff.png) 1회 로드.
let _flameTex = null;
function _getFlameTex() {
  if (_flameTex) return _flameTex;
  _flameTex = new THREE.TextureLoader().load('assets/fx/smoke_puff.png');
  _flameTex.colorSpace = THREE.SRGBColorSpace;
  return _flameTex;
}

// 후방 화염 제트: cross-plane (xz + yz) textured. 짧고 컴팩트, 흰-핫 코어 또렷.
function _makeFlameGroup(x = 0, y = 0.55, z = -3.05, size = 1.0) {
  const flame = new THREE.Group();
  flame.name = 'boostflame';
  flame.position.set(x, y, z);
  flame.visible = false;

  const FL = (KC.BOOST_FLAME_LENGTH ?? 0.85) * size;
  const FW = (KC.BOOST_FLAME_WIDTH  ?? 0.50) * size;
  const tex = _getFlameTex();
  // 3 layers — inner(흰-핫) → mid(주황) → outer(빨강). opacity 높여 또렷.
  const layers = [
    { name: 'flameinner', color: 0xffffff, width: FW * 0.45, len: FL * 0.60, opacity: 1.00 },
    { name: 'flamemid',   color: 0xffae3c, width: FW * 0.75, len: FL * 0.85, opacity: 0.85 },
    { name: 'flameouter', color: 0xff3a1a, width: FW * 1.00, len: FL * 1.00, opacity: 0.55 },
  ];
  for (const L of layers) {
    // 2 cross planes (xz + yz), 둘 다 길이축 -z로 뻗음.
    for (let rot = 0; rot < 2; rot++) {
      const geo = new THREE.PlaneGeometry(L.width, L.len);
      // PlaneGeometry 기본 xy. rotateX(π/2) → xz 평면 (length=z, width=x).
      geo.rotateX(Math.PI / 2);
      // 두번째 plane은 z축 기준 90° 회전 → yz 평면 (length=z, width=y).
      if (rot === 1) geo.rotateZ(Math.PI / 2);
      ...
      m.name = L.name;
      m.position.z = -L.len / 2;   // base at port, tip at -L.len
      flame.add(m);
    }
  }
  return flame;
...
// 후방 배기 포트 2개 (좌·우). 차 밑 둥근 풀 ❌, 후방 직진 제트.
function addBoostFlameTo(group) {
  const y = 0.55, z = -3.05;
  for (const x of [-0.45, 0.45]) group.add(_makeFlameGroup(x, y, z, 1.0));
}

function addDualBoostFlameTo(group, xGap = 0.45, y = 0.55, z = -3.05) {
  // 호환용. 2포트 동일.
  for (const x of [-xGap, xGap]) group.add(_makeFlameGroup(x, y, z, 1.0));
}
