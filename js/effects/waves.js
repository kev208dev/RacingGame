// Waves — vanilla JS port of React Bits <Waves /> component.
// Animated perlin-noise wave field on a canvas, with cursor interaction.
// API:
//   const handle = createWavesBackground(parentEl, options);
//   handle.dispose();
//
// Options mirror the React component props (lineColor, backgroundColor,
// waveSpeedX, waveSpeedY, waveAmpX, waveAmpY, xGap, yGap, friction, tension,
// maxCursorMove).
...
export function createWavesBackground(parentEl, options = {}) {
  const cfg = {
    lineColor: options.lineColor ?? 'rgba(255,255,255,0.55)',
  ...
  function movePoints(time) {
    const { waveSpeedX, waveSpeedY, waveAmpX, waveAmpY, friction, tension, maxCursorMove } = cfg;
    lines.forEach(pts => {
      pts.forEach(p => {
